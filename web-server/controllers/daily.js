...
module.exports = {
    ...
    list: function (req, res, next) {
        ...
        if (subFilter.view == "*") {
            ...
            teamRequire = {team_id: subFilter.view};
        }
        else {
            res.success();
            return;
        ...
        async.series([
            ...
            function (err, result) {
                ...
                sendResult = Utils.pakoZip(JSON.stringify(sendResult));
            ...
            DailySell.find({
                ...
                for (var i = 0; i < DailySellFound.length; i++) {
                    ...
                    if(allTeamName.indexOf(teamName) == -1) {
                        allTeam.push({teamName: teamName, teamID: teamID});
                ...
                DailySell.find(lossFindRequire).sort({date: -1}).exec(function (err, lossList) {
                    ...
                    allTeam.forEach(function(team) {
                        ...
                        sendResult.push({
                            team: {teamName: teamName, teamId:teamID},
    ...
    PopupList: function (req, res, next) {
        ...
        async.series([
                // 在商品列表中 通过卖家SKU找到相应的仓库SKU、店铺名和小组名
                function (callB) {
                    Merchandise.find({team_id:teamID, state:{$in:[2,3]}})
                        .populate("product_id")
                        .populate("shop_id")
                        .populate("team_id")
                        .exec(function (err, foundResult) {
                        ...
                        for (var i = 0; i < foundResult.length; i++) {
                            ...
                            if (row.product_id) map_seller2name[key] = row.product_id.name_cn;
                            lossFindRequire.$or.push({asin: asin,seller_sku: sellerSku})
                        }
                        callB(null);
                    });
                },
