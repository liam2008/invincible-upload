(function () {
    ...
    app.controller('reviewCtrl', ['$scope', 'netManager', 'DTOptionsBuilder',
        function ($scope, netManager, DTOptionsBuilder) {
            ...
            function getReviewData(sendData) {
                netManager.get('/appraise/EVALDetail', sendData).then(function (res) {
                    ...
                    //线图
                    var lineStars = res.data.lineStars;
                    var allPv = [];//全部数据，购买+非购买
                    var data = [];
                    for (var i = 0, iLen = lineStars.lineTime.length; i < iLen; i++) {
                        if (lineStars.isPv[i] === null && lineStars.isNotPv[i] === null) {
                            allPv.push(null)
                        } else {
                            allPv.push(lineStars.isPv[i] + lineStars.isNotPv[i])
                        }
                    }
                    data.push(allPv, lineStars.isPv, lineStars.isNotPv);
                    console.log('data', data);
                    $scope.lineData = {
                        series: ['全部', '购买', '非购买'],
                        data: data,
                        labels: lineStars.lineTime
                    };
