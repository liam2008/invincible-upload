    ...
    list: function(req, res, next) {
        Role.find({}, function(err, allRoles) {
            ...
            allRoles.forEach(function(row) {
                mapRoleType2Id[row.type] = row._id;
            });
            ...
            // 组员和助理没有管理权限
            if (agent.role == null
                || agent.role.type == null
                || agent.role.type == "member"
                || agent.role.type == "assistant") {
                res.success();
                return;
            }
            ...
            switch (roleType) {
                ...
                // 经理可以管理经理以下的(不包括其他经理)的人员
                case "manager": {
                    findCondition = {
                        role: {
                            $not: {
                                $in: [ mapRoleType2Id["admin"], mapRoleType2Id["manager"] ]
                            }
                        }
                    };
                    break;
                ...
                case "director": {
                    findCondition = {
                        role: {
                            $in: [ mapRoleType2Id["leader"], mapRoleType2Id["member"] ]
                        }
                    };
                    break;
                }
